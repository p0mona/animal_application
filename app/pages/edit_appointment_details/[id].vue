<template>
  <FullWidthLayout>
    ...
    <div v-else-if="appointment" class="space-y-6">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6" v-if="appointment.patient_id && typeof appointment.patient_id === 'object'">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">
...
const saveChanges = async () => {
  try {
    ...
    const response = await $fetch(`http://localhost:3001/appointments/${appointmentId}/medical`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
